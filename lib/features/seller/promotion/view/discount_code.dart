        ...
        subtitle: Column(
          ...
          children: [
            (code.discountType == 'percent')
                ? Text(
                  'Giảm ${code.value}% cho đơn từ ${code.minOrder.toStringAsFixed(0)}đ',
                )
                : Text(
                  'Giảm ${code.value}k cho đơn từ ${code.minOrder.toStringAsFixed(0)}đ',
                ),
            Text(
              'HSD: đến ${code.expiredDate.toLocal().toString().split(' ')[0]}',
            ),
          ],
        ),
      trailing:  IconButton(
